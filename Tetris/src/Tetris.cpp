...
void Tetris::draw_map() {
	for (int i = 1; i < HEIGHT + 2; i++) {
		cursor_move_to(0, i);
		for (int j = 0; j < WIDTH + 2; j++) {
			cursor_draw_block(m_color_map[i][j]);
		}
	}
}
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			if (points[i][status][j] == 1) {
				cursor_move_to(graph.m_prev_x + j, graph.m_prev_y + i);
				cursor_draw_block(CLEAR);
			}
		}
	}
...
void Tetris::draw_cur_graph(Shape& graph) {
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			if (points[i][status][j] == 1) {
				cursor_move_to(graph.m_x + j, graph.m_y + i);
				cursor_draw_block(graph.m_color);
			}
		}
	}
	pthread_mutex_unlock(&g_draw_mutex);
...
void Tetris::draw_score() {
	cursor_move_to(WIDTH + 4, HEIGHT - 2);
	cout << "Level: " << m_level;
	cursor_move_to(WIDTH + 4, HEIGHT - 1);
	cout << "Score: " << m_score;
}
